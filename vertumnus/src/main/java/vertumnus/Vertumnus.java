...
public class Vertumnus {
	...
	public static void main(String[] args) {
		try {
			sb = new StringBuilder(); // wichtig, muss in Methode statt finden
			System.out.println("VERTUMNUS 1.0.0 - Live Update");
			VertumnusCLI cli = CliFactory.parseArguments(VertumnusCLI.class, args);
			Updater u = new Updater();
			u.setVerzeichnis(cli.getDirectory());
			u.setKeepMajorVersion("minor".equalsIgnoreCase(cli.getLine()));
			String dn = null, nv = null;
			if (cli.getStage().contains("1")) {
				...
				sb.append("NV=" + nv + ";");
				System.out.println("Update auf " + nv); // TODO NLS
			}
			if (nv != null && cli.getStage().contains("2")) {
				dn = u.load();
				sb.append("dn=" + dn + ";");
			}
			if (dn != null && cli.getStage().contains("3")) {
